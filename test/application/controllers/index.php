    ...
    public function patient_mail(){
     ...
     public function contact(){
      ...
      public function career_apply(){
            ...
            $resume = '';
            $this->index_model->data = $this->input->post();
            $this->resume = array('upload_path' => RESUME_PATH, 'allowed_types' => '*');
            $this->upload->initialize($this->resume);
           
            if (!empty($_FILES['resume']['name']) && $this->upload->do_upload('resume')) {
                $upload_data = $this->upload->data();
               
                $file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
                $gen_filename = "resume".rand ( 1000000 , 9999999 ).".".$file_Ext;
                rename($upload_data['full_path'],$upload_data['file_path'].$gen_filename);
                
                $this->doctors_model->data['resume'] = $resume =  $gen_filename;
            }
            ...
            if($this->index_model->insert('careers')){
                ...
                $message .= '<br> Email :'.$this->input->post('email');
                $message .= '<br> Phone :'.$this->input->post('phone');
                $message .= '<br> Current Location :'.$this->input->post('current_location');
                $message .= '<br> Qualification :'.$this->input->post('qualification');
                $message .= '<br> Current Designation :'.$this->input->post('current_designation');
                $message .= '<br> Resume :'.base_url().RESUME_PATH.$resume;
